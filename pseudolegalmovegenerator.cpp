...
QList<Move> PseudoLegalMoveGenerator::GenerateMoves (Board *board, bool includeQuietMoves, bool includeUnderPromotions) {
    this->board = board;
...
bool PseudoLegalMoveGenerator::SquareAttacked (int attackSquare, int attackerColour) {
    ...
    if (PMD.kingDistance[opponentKingSquare][attackSquare] == 1) {
        return true;
    }
    ...
    for (int dir = startDirIndex; dir < endDirIndex; dir++) {
        ...
        int n = PMD.numSquaresToEdge[attackSquare][dir];
    ...
    // Knight attacks
    auto knightAttackSquares = PMD.knightMoves[attackSquare];
    for (int i = 0; i < knightAttackSquares.size(); i++) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (PMD.numSquaresToEdge[attackSquare][PMD.pawnAttackDirections[friendlyColourIndex][i]] > 0) {
            // move in direction friendly pawns attack to get square from which enemy pawn would attack
            int s = attackSquare + PMD.directionOffsets[PMD.pawnAttackDirections[friendlyColourIndex][i]];
...
void PseudoLegalMoveGenerator::Init () {
    moves.clear();
    moves.reserve(64);
...
void PseudoLegalMoveGenerator::GenerateKingMoves () {
...
void PseudoLegalMoveGenerator::GenerateSlidingMoves () {
...
void PseudoLegalMoveGenerator::GenerateSlidingPieceMoves (int startSquare, int startDirIndex, int endDirIndex) {
...
void PseudoLegalMoveGenerator::GenerateKnightMoves () {
...
void PseudoLegalMoveGenerator::GeneratePawnMoves () {
...
void PseudoLegalMoveGenerator::MakePromotionMoves (int fromSquare, int toSquare) {
