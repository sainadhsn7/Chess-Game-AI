...
void GameManager::start(){
    ...
    updateTimer->start(20); // Approximately 60 FPS
}
void GameManager::update () {
    ...
    int nextInterval = qMax(0, 20 - elapsed);
...
void GameManager:: onMoveChosen(Move &move){
    ...
    bool IsCapture = (board->Square[move.TargetSquare()] != Piece::None) || (move.IsEnpassant());
    ...
    if(moveGenerator.InCheck()){
        ...
        if(IsCapture){
            ...
            if(board->ColourToMove == board->OpponentColour) soundManager->move_self();
            else soundManager->move_opponent();
        }
    }
...
void GameManager::createPlayer(Player *&player, PlayerType playerType)
{
    ...
    connect(player, &Player::moveChosen, this, &GameManager::onMoveChosen);
}
